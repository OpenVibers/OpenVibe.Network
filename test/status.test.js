...
(async () => {
    ...
    assert.strictEqual(by.wiki.status, 'not-running');
    assert.strictEqual(by.wiki.label, 'not running (placeholder)');
    ...
    assert.ok(/id="svc-wiki"[\s\S]*?not running \(placeholder\)/.test(html));
