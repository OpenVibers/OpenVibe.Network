...
| **Session pick-up (lazy SSO)** | `sso-client.js` (link hand-off, FedCM RP), `GET /sso/check` + `ov_sso`, FedCM IdP (`server/auth/fedcm.js`), jwt-bearer grant | Nothing happens at sign-in. Layers, cheapest first: (1) **link hand-off** — while signed in, a click to another OpenVibe site goes through that site's silent sign-in and lands on the link (every browser, no third-party cookies; a site that already has a session answers in one redirect); (2) a site opened without a session: hint → hidden `/sso/check` → **FedCM** (browser-native, works with third-party cookies blocked; native chip once per site, silent re-auth after; a no-op for guests via the login-status API). `/sso/fanout` (the old chain) exists only behind the explicit "Sign in / out everywhere" buttons. |
